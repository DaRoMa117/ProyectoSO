...
{
    public partial class PantallaDosManual : Form
    {
        Socket server;
        int numSegundaManual;
        int numPartida;
        int posicion;
        int numSegunda;
        int vidas = 3;
        ...
        public PantallaDosManual(Socket server, int numPartida)
        {
            InitializeComponent();
            this.server = server;
            this.numSegundaManual = numPartida;
            this.numPartida = numPartida;
            this.posicion = numPartida;
            this.numSegunda = numPartida;
        }

        private void PantallaDosManual_Load(object sender, EventArgs e)
        {
        ...
        private void botonEnviar_Click(object sender, EventArgs e)
        {
            if (textChat.Text.Trim() != string.Empty)
            {
                ...
                string mensajeChat = "9/" + numSegundaManual + "/" + numPartida + "/" + text + "/segunda/" + numSegunda;
