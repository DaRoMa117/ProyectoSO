...
int posicionUsuario (ListaConectados *lista, char usuario[20]){ //SEGURAMENTE SE BORRARÁ
	int i = 0;
	int encontrado = 0;
...
void iniciarUsuario (MYSQL *conn, MYSQL_RES *resultado, MYSQL_ROW row, char usuario[20], char contrasena[20], char respuesta[512], ListaConectados *lista, int sock){
	...
	if (row == NULL)
		...
		if (strcmp(contrasena,row[0]) == 0)
			...
			sprintf(respuesta, "%s: contraseÃ±a incorrecta", usuario);
	}
...
void *AtenderCliente(void *socket){
	...
	{		
		...
		if (codigo == 0){
			...
			strcpy (contrasena, p);
			pthread_mutex_lock( &mutex );
			iniciarUsuario(conn, resultado, row, usuario, contrasena, respuesta, &lista, sock_conn);
			pthread_mutex_unlock( &mutex);
		}
		else if (codigo == 3){
